...
const PALACE_ROLES: Record<string, string> = {
  '命宫': '自我、性格、先天格局', '兄弟宫': '兄弟关系、合伙人', '夫妻宫': '感情关系、婚姻状态',
  '子女宫': '子女缘分、下属关系', '财帛宫': '财运来源、收入方式', '疾厄宫': '身体健康、意外',
  '迁移宫': '外出机遇、人际格局', '交友宫': '朋友圈、贵人、小人', '官禄宫': '事业成就、社会地位',
  '田宅宫': '不动产、家庭环境', '福德宫': '精神享受、内心福分', '父母宫': '父母关系、文书契约',
};
...
export default function InsightPanel({ chart, selectedPalace, selectedSiHua }: InsightPanelProps) {
  ...
  useEffect(() => {
    ...
    sendMessage(`请重点分析【${selectedPalace.name}】（主管：${role}），该宫主星为${starDesc}，按以下结构输出：\n\n**【宫位定性】**\n${selectedPalace.name}在命盘中的意义，以及这种星曜配置的整体判断。\n\n**【主星解读】**\n主星在此宫的倪海夏体系解读，引用具体观点。\n\n**【三方四正联动】**\n三方四正宫位对此宫的影响。\n\n**【实际建议】**\n基于此宫的具体建议。`, true);
  }, [selectedPalace]);
  ...
  const streamResponse = async (apiMessages: { role: 'user' | 'assistant'; content: string }[]) => {
    ...
    } catch { setMessages(prev => [...prev, { role: 'assistant', content: '解读失败，请稍后重试。' }]); }
  ...
  return (
    <div className="flex flex-col h-full rounded-xl overflow-hidden card-glass">
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-4 min-h-0">
        {messages.length === 0 && <div className="flex flex-col items-center justify-center h-full text-center"><Sparkles size={36} strokeWidth={1.5} className="mb-3" style={{ color: 'var(--t-gold)', opacity: 0.1 }} aria-hidden /><p className="text-[10px] animate-pulse" style={{ color: 'var(--t-faint)' }}>命格解读生成中…</p></div>}
        <AnimatePresence initial={false}>{messages.map((msg, i) => {
          ...
          return (<motion.div key={i} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }}>
            <div className="text-[9px] tracking-widest mb-2 flex items-center gap-1.5" style={{ color: 'var(--t-faint)' }}><Sparkles size={10} strokeWidth={1.8} style={{ color: 'var(--t-gold)', opacity: 0.4 }} aria-hidden />命理解读</div>
