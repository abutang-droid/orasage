...
interface ChartSummaryProps { chart: ZiweiChart; }
...
export default function ChartSummary({ chart }: ChartSummaryProps) {
  ...
  for (const palace of chart.palaces) {
    for (const star of palace.stars) {
      if (star.siHua) siHuaSummary.push({ name: star.name, siHua: star.siHua, palaceName: palace.name });
    }
  }
  siHuaSummary.sort((a, b) => ['禄', '权', '科', '忌'].indexOf(a.siHua) - ['禄', '权', '科', '忌'].indexOf(b.siHua));
    ...
    <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35, duration: 0.55, ease: [0.25, 0.1, 0.25, 1] }}>
      <div className="card-glass rounded-xl p-5">
        ...
        <div className="flex flex-wrap items-start gap-4">
          <div>
            <div className="text-[9px] mb-1.5" style={{ color: 'var(--t-faint)', opacity: 0.85 }}>{t('summary.minggong.stars')}</div>
            <div className="flex items-center gap-1">{mingStars.length > 0 ? mingStars.map(s => (<span key={s} className="font-bold text-lg" style={{ color: 'var(--t-gold)' }}>{s}</span>)) : <span className="text-sm" style={{ color: 'var(--t-faint)' }}>空宫</span>}</div>
            {nature && <div className="text-[10px] mt-1" style={{ color: 'var(--t-gold)', opacity: 0.5 }}>{nature}</div>}
          </div>
          {keywords.length > 0 && <div><div className="text-[9px] mb-1.5" style={{ color: 'var(--t-faint)', opacity: 0.85 }}>{t('summary.personality')}</div><div className="flex flex-wrap gap-1.5">{keywords.map(k => (<span key={k} className="text-[10px] px-2 py-0.5 rounded-full" style={{ color: 'var(--t-gold)', background: 'rgba(212,168,67,0.08)', border: '1px solid rgba(212,168,67,0.15)' }}>{k}</span>))}</div></div>}
          <div className="ml-auto text-right">
            <div className="text-[9px] mb-1" style={{ color: 'var(--t-faint)', opacity: 0.85 }}>{t('summary.wuxing')}</div>
            <div className="text-[11px]" style={{ color: 'var(--t-text2)' }}>{chart.wuxingJuName}</div>
            {currentDx && <div className="text-[11px] text-purple-500 mt-0.5">{currentDx.startAge}~{currentDx.endAge}{t('chart.center.years')} · {currentDx.palaceName}</div>}
          </div>
        </div>
        <div className="mt-4 pt-3 flex flex-wrap gap-x-4 gap-y-1 text-[10px]" style={{ borderTop: '1px solid var(--t-border)', color: 'var(--t-faint)' }}>
          <span>{t('summary.solar')} {chart.birthInfo.year}-{chart.birthInfo.month}-{chart.birthInfo.day}</span>
          <span>{t('summary.lunar')} {chart.lunarInfo.lunarYear}年{chart.lunarInfo.isLeapMonth ? t('summary.leap') : ''}{chart.lunarInfo.lunarMonth}月{chart.lunarInfo.lunarDay}日</span>
    ...
    {siHuaSummary.length > 0 && (<motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.55, duration: 0.5, ease: [0.25, 0.1, 0.25, 1] }}>
      <div className="card-glass rounded-xl p-4">
        ...
        <div className="grid grid-cols-2 gap-2">{siHuaSummary.map(({ name, siHua, palaceName }) => {
          ...
          return (<div key={name + siHua} className="flex items-center justify-between px-3 py-2 rounded-lg text-[10px]" style={{ background: c.bg, border: `1px solid ${c.border}`, color: c.text }}>
    ...
    {patterns.length > 0 && (<motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.72, duration: 0.5, ease: [0.25, 0.1, 0.25, 1] }}>
      <div className="card-glass rounded-xl p-4">
        ...
        <div className="space-y-2">{patterns.map((p, i) => {
          ...
          return (<motion.div key={i} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.8 + i * 0.08 }} className="card-inner rounded-lg p-3">
            <div className="flex items-center gap-2 mb-1.5"><div className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${st.dot}`} /><span className={`text-[11px] font-medium ${st.label}`}>{p.name}</span><div className="flex gap-1 ml-auto">{p.palaces.slice(0, 2).map(pa => (<span key={pa} className={`text-[8px] px-1.5 py-px rounded-full border ${st.badge}`}>{pa.replace('宫', '')}</span>))}</div></div>
            <p className="text-[10px] leading-relaxed pl-3.5" style={{ color: 'var(--t-text2)' }}>{p.description}</p>
            {p.conditions && <div className="mt-2 pl-3.5 space-y-0.5">
              {p.conditions.required.length > 0 && <div className="text-[9px] leading-relaxed" style={{ color: 'var(--t-text2)', opacity: 0.85 }}><span className="font-medium" style={{ color: 'var(--t-gold)' }}>{t('summary.pattern.required')}</span><span style={{ opacity: 0.6 }}> · </span>{p.conditions.required.join('、')}</div>}
              {p.conditions.bonus && p.conditions.bonus.length > 0 && <div className="text-[9px] leading-relaxed" style={{ color: 'var(--t-text2)', opacity: 0.85 }}><span className="font-medium text-emerald-500">{t('summary.pattern.bonus')}</span><span style={{ opacity: 0.6 }}> · </span>{p.conditions.bonus.join('、')}</div>}
              {p.conditions.breaking && p.conditions.breaking.length > 0 && <div className="text-[9px] leading-relaxed" style={{ color: 'var(--t-text2)', opacity: 0.85 }}><span className="font-medium text-orange-500">{t('summary.pattern.breaking')}</span><span style={{ opacity: 0.6 }}> · </span>{p.conditions.breaking.join('、')}</div>}
            </div>}
    ...
    <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.88, duration: 0.5, ease: [0.25, 0.1, 0.25, 1] }}>
      <div className="card-glass rounded-xl p-4">
        ...
        <div className="grid grid-cols-3 gap-2">{chart.daXians.slice(0, 9).map((dx, i) => {
          ...
          return (<div key={i} className="text-[10px] px-2 py-2 rounded-lg text-center transition-colors" style={{ border: isCurrent ? '1px solid rgba(147,51,234,0.4)' : '1px solid var(--t-border)', background: isCurrent ? 'rgba(147,51,234,0.08)' : 'transparent', color: isCurrent ? '#a78bfa' : 'var(--t-faint)' }}>
