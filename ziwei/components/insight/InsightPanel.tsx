      ...
      <div className="flex-shrink-0 px-3 pb-3 pt-2" style={{ borderTop: '1px solid var(--t-border)' }}>
        <div className="flex gap-2">
          <input type="text" value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && !e.shiftKey && handleSend()}
            placeholder={t('insight.placeholder')} disabled={loading}
          ...
          <button onClick={handleSend} disabled={loading || !input.trim()}
            ...
            style={{ background: 'rgba(212,168,67,0.15)', border: '1px solid rgba(212,168,67,0.25)', color: 'var(--t-gold)' }}>{loading ? '…' : t('insight.submit')}</button>
        </div>
      </div>
    </div>
  );
