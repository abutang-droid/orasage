...
- Default terminals: `auth-service` (:3101), `main` (:3100, locale path `/zh-CN`), `shop` (:3102), `ziwei` (:3111, redirects to `/chart`). Start the others on demand:
...
`deploy/lib/ssh-setup.sh` auto-starts `cloudflared access tcp` when `SSH_HOST` matches `ssh.*` or `SSH_TUNNEL_HOSTNAME` is set. Verify: `source deploy/lib/ssh-setup.sh && setup_ssh_key && test_ssh_connection`.
