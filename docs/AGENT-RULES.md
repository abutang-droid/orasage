...
| 终端 | 导航形态 |
|------|----------|
| **PC（≥1024px）** | 顶部水平菜单：英文 **Shop → Insights → Origins → Readings**；中文 **首页 / 商城 / 玄析 / 造物 / 测算** + Search · Me · Cart + 语言切换（商城、玄析无二级；道藏与名人案例在测算二级） |
...
4. 自动模式：把 `cloudflared access ssh --hostname %h` 写入 `~/.ssh/config.orasage-deploy`（已验证可用于 `ssh.orasage.com`）。调用方用 `ssh $SSH_OPTS` 时不会把带空格的 ProxyCommand 拆碎。
