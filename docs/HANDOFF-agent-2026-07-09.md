...
**Git：** `main` @ **`c4bb7df`**（含本文档；功能代码至 PR #251）
...
### 5.4 订单通知（#235 代码已合入，生产 env 待确认）

| 项 | 说明 |
|----|------|
| **代码** | `auth-service/src/lib/order-notify.ts` — 新订单/支付成功 → Telegram + Resend 邮件（未配置则静默跳过）；admin 订单角标 API 已有 |
| **VPS env** | `TELEGRAM_BOT_TOKEN`、`TELEGRAM_CHAT_ID`、`RESEND_API_KEY`、`ORDER_NOTIFY_EMAIL_TO` 是否已配 **未在本日验证** |

---

## 10. 平台 Backlog 与待用户确认项（**下一 Agent 必读，勿丢失**）

> 以下为用户在规划讨论中列出的条目、现状评估、建议拆分与确认结论。  
> 编号与用户原列表一致（2、3、7、8+9+11、10、13 等）。**均未在本会话实现**，除非注明已有部分代码。
...
|---|------|
| **13** | **最后做**整站视觉测试（移动端为主） |
| **3** | 倾向 **新加坡 + 圣保罗双节点**（见下方 Agent 建议：分阶段，勿一步 active-active） |
| **2** | **外部第三方**开放 API（非仅自有 app）；方案见 §10.2 Agent 建议 |
| **10** | **路线 A**：自建 `events` 表 + `packages/` 统一 SDK → 喂 **7b** |
| **8+9+11** | 合并「**消息中枢**」；转接 **Telegram**；邮件 **Resend**；顺序 **11 → 9 → 8** |
| **7** | 顺序 **7a → 7c → 7b（等 10）→ 7d**；**7d** 见下方（非一上来全复式记账） |

### 7. CMS/后台权限 + 数据统计 + 资金账户 + 用户钱包

**现状：**

- CMS 登录只认 JWT `role === 'admin'`，无子账号、无细分权限。
- admin **无用户管理页**。
- **无数据统计后台**。
- `users` 表无钱包/余额字段（仅有紫微问答额度等服务额度类字段）。

**建议拆成 4 个子项：**

| 子项 | 内容 | 依赖 / 备注 |
|------|------|-------------|
| **7a 权限** | auth-service role 扩展为多角色 + **权限点**（按应用/功能授权）；admin 加**子账号管理页**；CMS 按角色控制 collection 读写 | Phase D 已有 `admin \| shop_ops \| content_ops`（#247），但**无权限点、无子账号 UI** |
| **7b 数据统计后台** | 运营看板（订单、用户、转化等） | **依赖 §10 行为统计**，建议排在 **10 之后** |
| **7c 用户钱包** | `users` 关联新表 `wallets`（余额、币种）+ **流水表**；只建模型 + internal API + admin 可见；**前台不展示**（前期不对外） |  greenfield |
| **7d 资金账户管理（平台侧）** | 平台收支视角 | **含义待用户定义** — Stripe 收款流水只读？还是平台自建复式记账？ |

**✅ 已确认：**

1. 实施顺序：**7a → 7c → 7b（等 #10）→ 7d**
2. **7d 含义（分阶段，用户问「平台自建记账？」）**
   - **7d-v1（推荐先做）**：Stripe 流水 **只读同步**（Charges / Refunds / Payouts / Balance）+ admin 对账视图；与 7c 用户钱包 **分开**（7c = 用户侧余额，7d = 平台侧现金视角）
   - **7d-v2（按需）**：若要做 affiliate 分润、多币种结算、内部成本分摊，再加 **轻量平台 ledger**（复式分录表），**不要**第一期就上完整财务 ERP
   - **结论**：7d **不等于**一上来「全自建复式记账」；先 Stripe 镜像 + 对账，ledger 视业务再开

---

### 8 + 9 + 11. 在线 IM 客服 + 投诉工单 + 订单提醒 → 建议合并为「**消息中枢**」专项

**现状：**

- 已有 `contact_messages` 表 + main 联系表单 + admin `/messages` 工单页。
- **订单通知**：#235 已写 `order-notify.ts`（TG + Resend），但 env 可能未配；用户感知上可能仍「无通知」。
- **无**站内 IM、无 TG 双向桥接、无用户端工单回复查看。

**方案理解（共享 Telegram 通道底座）：**

| 顺序 | 项 | 工程量 | 说明 |
|------|-----|--------|------|
| **11（建议先做）** | 订单提醒 | 小 | 支付成功 → TG Bot + 邮件 + admin 角标（**大半已有 #235**，补 env + 验收） |
| **9** | 投诉/工单 | 中 | 扩展 `contact_messages`：分类（投诉/退款/bug）、关联订单号、状态流转、**用户端可查看回复** |
| **8** | 在线 IM | 大 | 站内浮窗；消息双向桥接 TG（用户 → TG 群/机器人，运营 TG 回复 → 站内）；需 WebSocket 或轮询 + TG Bot webhook |

**✅ 已确认：**

1. 转接目标：**Telegram**
2. 邮件：**Resend**（与现有 `order-notify.ts` 一致）
3. 顺序：**11 → 9 → 8**

**备注：** 巴西/东南亚用户触达可后续在消息中枢加 WhatsApp Business / LINE 适配层，底座仍走同一 `notifications` 表 + 渠道 driver。

---

### 10. 行为统计

**现状：** 全站**零埋点**（ziwei 装了 `@vercel/analytics` 但未启用）。

**两条路线：**

| 路线 | 说明 |
|------|------|
| **A) 自建轻量埋点** | `events` 表 + 各应用统一上报 SDK（放 `packages/`）；数据归己，可喂 **7b 统计后台** |
| **B) Plausible/Umami 自托管** | 省开发，与 7b 打通需二次开发 |

**倾向：** A + 可选叠加 Umami 看大盘。

**合规：** 面向巴西用户注意 **LGPD**；自建埋点默认**不存 IP 明文**、加匿名化。

**✅ 已确认：** **路线 A**（自建 `events` + `packages/` SDK，喂 7b）。Umami 可后叠加看大盘，非阻塞。
...
- 商品 API（auth-service `/api/products`）与 CMS 内容 API **已公开可读**。
- 缺：面向第三方的正式能力 — **API key 鉴权**、开放下单/结账、文档；partner webhook（`order.paid`）**未实现**。

**✅ 已确认：** 目标为 **外部第三方**（非仅整理自有 app 内网 API）。

**Agent 建议方案（分三期，均落在 auth-service + shop）：**

| 期 | 能力 | 说明 |
|----|------|------|
| **2-α** | Partner 注册 + **API Key**（`partner_apps` 表：name、key_hash、scopes、webhook_url、rate_limit） | 鉴权：`Authorization: Bearer pk_live_…` 或 `X-Api-Key`；scopes 如 `catalog:read` |
| **2-β** | **代下单 + 支付链接** | `POST /api/partner/v1/orders`（sku、qty、partner_ref、return_url）→ 返回 `checkout_url`（shop 托管结账，Stripe/mock）；**不把**用户密码交给第三方 |
| **2-γ** | **Webhook** | HMAC-SHA256 签名事件：`order.paid`、`order.shipped`、`order.refunded`；重试 + idempotency key |
| 文档 | OpenAPI 3 + 沙箱 key（`pk_test_`） | 与 `PAYMENT_MODE=mock` 联调 |

**不推荐第一期：** OAuth 用户委托、嵌入式 iframe 全结账 UI、Stripe Connect 分账（除非明确要做 marketplace）。

**与现有架构关系：** 商品仍读 auth PG；订单仍写 `user_orders`（加 `partner_id` / `partner_ref` 列）；支付仍走 shop webhook → auth internal。

---

### 3. 亚洲 + 南美各设一个服务节点（基础设施决策）

**现状：** 单台 GCP VPS（`34.75.40.67`，美东 us-east1 段）。

| 方案 | 说明 | 复杂度 |
|------|------|--------|
| **最低成本** | 单源站 + **Cloudflare CDN**（静态全球加速，动态回源）；改 DNS + 缓存头 | 低 |
| **真双节点** | GCP 新加坡 + 圣保罗各一台；PG 单点 → 主从/读写分离；`deploy/` 多目标改造 | 高 |

**✅ 用户倾向：** **新加坡 + 圣保罗双节点**。

**Agent 建议（分阶段，避免一步双活 PG）：**

| 阶段 | 做法 |
|------|------|
| **3-0（立刻可做）** | **Cloudflare** 前置全站（静态 + 缓存策略）；源站仍美东 VPS |
| **3-1** | **圣保罗** 计算节点（Next/Express 只读镜像或反向代理），**PostgreSQL 仍单主**（主库放离写流量最近区域，或维持美东） |
| **3-2** | **新加坡** 边缘/只读副本（或 Cloudflare Workers 缓存 catalog API） |
| **3-3（业务量大后）** | PG 跨区只读 replica + 明确写入口；**不要**第一期双主 |

巴西用户（pt-BR 第一期语言）优先保证 **shop/auth API 圣保罗 RTT**；内容/CMS 可 CDN 全球加速。

---

### 13. 整站视觉测试（移动端为主）

**理解：** 产出全站（**8 子域 × 移动/PC**）视觉审查报告 — 截图、逐页列布局/字号/按钮比例/排版问题；对照设计规范但不硬套；再按报告逐项修。

**建议时机：** 放在已知问题项（如 4/5/6/12 等，若存在）修完后再做，避免报告重复已知项。

**✅ 已确认：** **最后做**（等功能 backlog 与已知 UI 问题收敛后再出报告）。

---

### Backlog 条目索引（便于新 Agent 检索）

| # | 主题 | 状态 | 备注 |
|---|------|------|------|
| 2 | 商城开放 API（第三方） | **已确认方向** | 方案 2-α→γ；见 §10.2 |
| 3 | 多区域节点 | **已确认倾向** | 新+圣保罗；分阶段 3-0→3-3 |
| 7a | 权限/子账号 | 未做 | Phase D 粗粒度已有；#252 `/staff` 基础 UI |
| 7b | 统计后台 | 未做 | **等 #10** |
| 7c | 用户钱包 | 未做 | 在 7a 之后 |
| 7d | 平台资金账户 | **已定义 v1** | Stripe 只读对账；非首期全复式 |
| 8 | 在线 IM | 未做 | 消息中枢 **第三批** |
| 9 | 投诉工单 | 部分 | 消息中枢 **第二批** |
| 10 | 行为统计 | **路线 A 已确认** | events + packages SDK |
| 11 | 订单提醒 | **部分 #235** | 消息中枢 **第一批**；TG+Resend |
| 13 | 视觉测试报告 | **最后做** | — |

### 建议实施总顺序（2026-07-09 确认后）

```
11 → 9 → 8          消息中枢
7a → 7c → 10 → 7b → 7d   权限/钱包/埋点/统计/资金
2-α → 2-β → 2-γ    开放 API（可与 7 并行，视人力）
3-0 → 3-1 → …       多区域（可与 CDN/B′ 合并）
13                  整站视觉（收尾）
```
...
1. ~~向用户确认 §10 Backlog 中所有 ❓~~ **✅ 2026-07-09 已全部确认**（见 §10 汇总表）。
2. **更新** `docs/plans/shop-admin-redesign-v2.md` Phase 状态表 + 第一期语言说明。
3. **订单提醒 #11 验收**：VPS 配齐 Resend/TG env，走一单 mock pay 验证通知 + admin 角标。
4. **Stripe + 优惠码**（若用户要上真实支付）：核对 webhook 与 `amount_cents` 一致性。
5. **生产抽检**：优惠码 used_count、三语 switcher、商店 PDP/DIY 单返回。
