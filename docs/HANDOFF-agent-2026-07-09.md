...
> 以下为用户在规划讨论中列出的条目、现状评估、建议拆分与 **❓ 待确认问题**。  
...
1. 拆分实施顺序是否 **7a → 7c → 7b（等 10）→ 7d**？
2. **7d** 具体指：查看 Stripe 收款流水，还是平台自己记账的账户体系？
...
1. **a)** 转接目标定 **Telegram** 吗？（还是 WhatsApp / LINE — 考虑东南亚、巴西用户）
2. **b)** 邮件通道用什么？（需在 Secrets 配 SMTP 或 **Resend/SendGrid** — 代码侧已用 Resend 接口）
3. **c)** 顺序 **11 → 9 → 8** 是否可以？
...
**❓ 待确认：** 路线选 A、B 还是 A+Umami？

---

### 2. 商城对外开放 API（非代码 / 评估类）

**现状：**

- 商品 API（auth-service `/api/products`）与 CMS 内容 API **已公开可读**。
- 缺：面向第三方的正式能力 — **API key 鉴权**、开放下单/结账（或可嵌入购买页）、文档；roadmap 中 partner webhook（`order.paid`）**未实现**。

**❓ 待确认：** 「别的应用」指 **自有未来新应用**（整理内网 API + 文档即可），还是 **外部第三方**（需 API key + 限流 + webhook，工程量差很多）？
...
| # | 主题 | 状态 | 阻塞 |
|---|------|------|------|
| 2 | 商城开放 API | 待确认范围 | 自有 vs 第三方 |
| 3 | 多区域节点 | 待确认方案 | CDN vs 双节点 |
| 7a | 权限/子账号 | 未做 | 用户确认顺序 |
| 7b | 统计后台 | 未做 | 依赖 #10 |
| 7c | 用户钱包 | 未做 | — |
| 7d | 平台资金账户 | 未定义 | 用户定义 7d |
| 8 | 在线 IM | 未做 | 在 9、11 之后 |
| 9 | 投诉工单 | 部分（contact_messages） | 在 11 之后 |
| 10 | 行为统计 | 未做 | 用户选路线 |
| 11 | 订单提醒 | **部分 #235** | env + 验收 |
| 13 | 视觉测试报告 | 未做 | 时机确认 |
