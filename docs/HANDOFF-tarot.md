...
| 祈福 | `/temple` → `WorshipScreen` / `BlessingScreen` / `TempleSelectPhase` |
...
旧路由 `/fortune` 已下线，重定向至 `/daily-fortune`（middleware + page redirect）。
...
| 项 | 状态 | 说明 |
|----|------|------|
| 三牌阵套装「查看订单」 | ✅ 已修 | 链 `orasage.com/zh-CN/profile/orders` |
| `/fortune` 独立页 | ✅ 已下线 | 重定向至 `/daily-fortune`；`lib/tarot/fortune.ts` 仍供 `TAROT_TIPS` 使用 |
| 自动化测试 | 无 | 改完跑 `JWT_SECRET=... npm run build`（`tarot/`） |
| MANTO 产品文档差距 | 未对齐 | 暗色粒子 Hero、首页单 CTA 等与现版浅色双卡片布局仍有差距 |
...
- [ ] 引导页选信仰 → 确认可见
- [ ] 每日运势：过场 + 翻牌 + 登录后回到报告
- [ ] 祈福 → 状态卡显示 + 参拜 → 每日运势；推荐商品 → 商城高亮 SKU
- [ ] CMS 改塔罗 Hero → 首页文案/图更新
- [ ] 登录用户占卜历史同步与用户中心详情链
- [ ] `/fortune` 访问自动跳转 `/daily-fortune`
