...
export function parseProductFormPayload(formData: FormData) {
  ...
  // USD-only list price; mirror into priceCents for NOT NULL / legacy readers.
  ...
  const saleUsdRaw = String(formData.get('salePriceUsd') ?? formData.get('salePriceYuan') ?? '').trim();
  ...
  const salePriceCents = salePriceCentsUsd;
