...
export default async function MessagesPage({ searchParams }: Props) {
  ...
  try {
    ({ messages } = await getContactMessages(activeStatus, activeCategory));
  } catch (err) {
  ...
  function filterHref(next: { status?: string; category?: string }) {
    ...
    return q ? `/ops/messages?${q}` : '/ops/messages';
  }
  ...
  return (
    <div className="admin-page">
      ...
      <header className="page-header">
        <h1>用户工单</h1>
        <p className="muted">
          联系表单 / 投诉退款工单 · 内部备注 + 用户可见回复（回复将邮件通知用户）。通知通道状态见{' '}
          <Link href="/integrations">集成状态</Link>。
        </p>
      </header>
